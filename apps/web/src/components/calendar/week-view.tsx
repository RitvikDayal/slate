    ...
    <div className="flex flex-col">
      ...
      {allDayEvents.length > 0 && (
        <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-slate-800">
          <div className="p-1 text-xs text-slate-500">All day</div>
          {days.map((day) => {
            ...
            return (
              ...
              >
                {dayEvents.map((e) => (
                  <div
                    key={e.id}
                    className="mb-0.5 truncate rounded bg-blue-500/20 px-1.5 py-0.5 text-xs text-blue-300"
                  >
                    {e.title}
                  </div>
                ))}
      ...
      <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-slate-800">
        ...
        {days.map((day) => (
          <div
            key={day.toISOString()}
            className="border-l border-slate-800 px-2 py-2 text-center"
          >
            ...
            <p
              className={cn(
                ...
                isSameDay(day, new Date()) &&
                  "mx-auto flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-white"
              )}
            >
              {format(day, "d")}
            </p>
          ...
          return (
            ...
            >
              ...
              {dayEvents.map((event) => (
                <div
                  key={event.id}
                  className="absolute left-1 right-1 overflow-hidden rounded border-l-2 border-blue-500 bg-blue-500/15 px-1.5 py-0.5"
